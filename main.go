...
// - ICMPResults: A channel to collect ICMP reachability results.
...
func NewTarget(domain string, numWorkers int) (*PortScanner, error) {
	...
	return &PortScanner{
		...
		Done:          make(chan bool, numWorkers*2+1),
...
// and ICMP scanning. It distributes the ports to be scanned through channels and waits for all results to be collected.
...
// - ICMP reachability results for each IP address.
func (t *PortScanner) Scan() {
	...
	for i := 0; i < t.NumWorkers; i++ {
		go adds.WorkerICMP(ipsChannel(t.IPs), t.ICMPResults, t.Done)
	}
	...
	doneCount := 0
	for doneCount < t.NumWorkers*2+1 {
		<-t.Done
		doneCount++
	}
	...
	_, err = file.WriteString("ICMP Results:\n")
	if err != nil {
		fmt.Printf("Error writing to file: %s\n", err)
		return
	}
...
// Helper function to create a channel for IP addresses.
func ipsChannel(ips []string) <-chan string {
